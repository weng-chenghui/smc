...
Lemma scalar_product_ok :
  ...
  ([:: Ret (one ((xa + sa) *d xb + (sa *d sb - ra) - yb - (xb + sb) *d sa + ra));
      Ret (one yb); Ret None],
   [:: Log 0 (Some (inl ((xa + sa) *d xb + (sa *d sb - ra) - yb)));
       Log 0 (Some (inr (xb + sb))); Log 1 (Some (inr (xa + sa)));
       Log 1 (Some (inr (xa + sa))); Log 1 (Some (inl (sa *d sb - ra)));
       Log 1 (Some (inr (xa + sa))); Log 1 (Some (inl yb)); Log 1 (Some (inr (xa + sa)));
       Log 1 (Some (inr sb)); Log 0 (Some (inl ra))]
  ).
