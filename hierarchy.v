...
From mathcomp.analysis Require Import realfun itv convex probability.
...
Notation UU1 := (Type@{high}).
Notation UU0 := (Type@{low}).
...
HB.mixin Record isMonadConvex_ {R : realType} (M : UU0 -> UU0) of Monad M := {
  convexity : forall T, isConvexSpace.axioms_ R (M T)
}.
...
Lemma r_of_pq_subproof (p q : {i01 R}) :
  Itv.spec `[0, 1] (p%:inum / (s_of_pq' p q)%:inum).
Proof.
Admitted.

Definition r_of_pq (p q : {i01 R}) : {i01 R} :=
  @Itv.mk _ _ _ (r_of_pq_subproof p q).

End s_and_r_of_pq.

HB.mixin Record isMonadConvex {R : realDomainType} (M : UU0 -> UU0) of Monad M := {
  choice : forall (p : {i01 R}) (T : UU0), M T -> M T -> M T ;
  (* identity axioms *)
  choice1 : forall (T : UU0) (a b : M T), choice 1%:i01 _ a b = a ;
  (* skewed commutativity *)
  choiceC : forall (T : UU0) p (a b : M T),
    choice p _ a b = choice (1 - p%:inum)%:i01 _ b a ;
  choicemm : forall (T : UU0) p, idempotent (@choice p T) ;
  (* quasi associativity *)
  choiceA : forall (T : UU0) (p q r s : {i01 R}) (a b c : M T),
    choice p _ a (choice q _ b c) = choice (s_of_pq p q) _ (choice (r_of_pq p q) _ a b) c }.

#[short(type=convexMonad)]
HB.structure Definition MonadConvex {R : realDomainType} := {M of isMonadConvex R M & }.
...
Lemma choice0 {R : realDomainType} (M : convexMonad R) (T : UU0) (a b : M T) :
  a <| 0%:i01 |> b = b.
Proof.
rewrite choiceC/= [X in _ <| X |> _](_ : _ = 1%:i01) ?choice1//; apply/val_inj => /=.
by rewrite subr0.
Qed.

Lemma subrDA (R : comRingType) (m n p : R) : n - (m + p) = n - m - p.
Proof. ring. Qed.

Lemma choiceA_alternative {R : realFieldType} (M : convexMonad R) :
  forall (T : UU0) (p q r s : {i01 R}) (a b c : M T),
    p = (r%:inum * s%:inum)%:i01 /\ 1 - s%:inum = (1 - p%:inum) * (1 - q%:inum) ->
    let bc := choice q _ b c in
    let ab := choice r _ a b in
    choice p _ a bc = choice s _ ab c.
Proof.
move=> T p q r s a b c [Hp Hs] bc ab.
have [Hs0|Hs0] := eqVneq s%:inum 0.
  have Hs0': s = 0%:i01 by apply/val_inj; rewrite /= Hs0.
  rewrite Hp /= Hs0' [X in a <| X |> bc](_ : _ = 0%:i01); last first.
    by apply/val_inj; rewrite /= mulr0.
  rewrite !choice0 /bc.
  suff -> : q = 0%:i01 by rewrite choice0.
  move=> ?.
  have:= Hs.
  have Hp0: p%:inum = 0 by rewrite Hp /= Hs0 mulr0.
  rewrite Hs0 Hp0 subr0 mul1r.
  move/eqP; rewrite addrC -subr_eq subrr eq_sym oppr_eq0 => /eqP Hq0.
  by apply/val_inj.
have s_of_pqE: s_of_pq p q = s.
  by apply/val_inj; rewrite /= -Hs subrDA subrr opprK add0r.
rewrite choiceA// s_of_pqE.
rewrite /ab; congr (a <| _ |> b <|_|> _). 
apply/val_inj=> /=.
rewrite -Hs subrDA subrr opprK add0r.
rewrite -[RHS]divr1; apply/eqP; rewrite eqr_div //.
by rewrite Hp mulr1.
Qed.
Arguments choiceA {_} {_} {_} _ _ _ _ {_} {_} {_}.
...
Fail Constraint Equality.axioms_.u0 < low.
Constraint low < Equality.axioms_.u0.
Fail HB.mixin Record isMonadFresh (S : eqType) (M : UU0 -> UU0) of Monad M :=
  { fresh : M (Equality.sort S) }.

Fail #[short(type=freshMonad)]
